                ...
                builder: (_, multiSelectMode, child) {
                    return Row(
                        ...
                        children: [
                            ...
                            if (multiSelectMode) 
                            IconButton(
                                onPressed: () => historyModel.toggleSelectAll(),
                                icon: Icon(
...
class _HistoryPageContentState extends State<HistoryPageContent> {
    ...
    List<Widget> buildPageSection(BuildContext context, int pageIndex) {
        ...
        int currentPageloadStartIndex = pageIndex == 0 ? 0 : historyModel.localHistoryMap.keys.elementAt(pageIndex - 1);

        int currentPageGroupSize = pageIndex == 0 ? 
            historyModel.localHistoryMap.keys.elementAt(pageIndex) : 
            historyModel.localHistoryMap.keys.elementAt(pageIndex) - historyModel.localHistoryMap.keys.elementAt(pageIndex - 1) + 1;
                        ...
                        Container(
                          ...
                          child: Row(
                              ...
                              children: [
                                  ...
                                  IconButton(
                                      onPressed: () {
                                          historyPageController.previousPage(duration: const Duration(milliseconds: 300), curve: Curves.easeOut);
                                      },
                                      icon: Icon(Icons.arrow_back_ios_new_outlined),
                                  ),
                          
                                  AnimatedBuilder(
                                      animation: historyPageController, 
                                      builder: (_, rangeButton) {
                          
                                          int startIndex = 0;
                                          int endIndex = 0;
                          
                                          if(historyPageController.hasClients){
                                            startIndex = historyModel.convertHistoryPageStartIndex(historyPageController.page!.toInt());
                                            endIndex = historyModel.convertHistoryPageEndIndex(historyPageController.page!.toInt());
                                          }
                                      ...
                                      child: IconButton(
                                        onPressed: (){
                                          if(historyPageController.hasClients){
                                            showHistoryDateRangeSelectDialog(context, historyPageController.page!.toInt()).then((index){
                                              if(index != null){
                                                historyPageController.jumpToPage(index);
                                              }
                                            });
                                          }
                                        }, 
                                  ...
                                  IconButton(
                                      onPressed: () {
                                          historyPageController.nextPage(duration: const Duration(milliseconds: 300), curve: Curves.easeOut);
                                      },
